<!DOCTYPE html>
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">

        <link rel="stylesheet" href="../../../DinhDangWebSite/Phieu/BangPhieu.css">
        <script defer src="../../../RangBuoc/Phieu/CauHinhBangPhieu.js"></script>
        
    </head>
    <body class="ThanTrang"> 
        <h1 class="BangLietKe">Bảng Phiếu tiếp nhận sinh viên thực tập</h1>
    <table id="CanChinhBangPXNSVTT" class="table table-striped BangPhieu">
            <thead>
                <tr class="TieuDeBangPhieu">
                    <th>Mã số phiếu xác nhận thực tập</th>
                    <th>Ngày hết hạn</th>
                    <th>Mã số sinh viên</th>
                    <th>Mã số giảng viên</th>
                    <th>Mã số cán bộ</th>
                    <th>Mã số đơn vị thực tập</th>
                    <th>Số đợt thực tập</th>
                </tr>
            </thead>
            <tbody>
                <?php
                    $DS = ds_PhieuTiepNhanSinhVien();
                    
                    while($row = mysqli_fetch_array($DS)){
                        //Kiểm tra các mã số cán bộ có rỗng không
                        if(empty($row['MSCB'])){
                            $mscb = '<span class="ThongSoNULL">null</span>';
                        }else{
                            $mscb = $row['MSCB'];
                        }
                        //Kiểm tra các mã số giảng viên có rỗng không
                        if(empty($row['MSGV'])){
                            $msgv = '<span class="ThongSoNULL">null</span>';
                        }else{
                            $msgv = $row['MSGV'];
                        }
                        //Kiểm tra các mã số cán bộ có rỗng không
                        if(empty($row['MaDVTT'])){
                            $madvtt = '<span class="ThongSoNULL">null</span>';
                        }else{
                            $madvtt = $row['MaDVTT'];
                        }

                        echo '<tr>';
                        echo    "<td>".$row['MSPXNTT']."</td>";
                        echo    "<td>".$row['ngayHetHan']."</td>";
                        echo    "<td>".$row['MSSV']."</td>";
                        echo    "<td>".$msgv."</td>";
                        echo    "<td>".$mscb."</td>";
                        echo    "<td>".$madvtt."</td>";
                        echo    "<td>".$row['STT']."</td>";
                        echo '</tr>';
                    }
                ?>
            </tbody>
            <tfoot>
                <tr class="TieuDeBangPhieu">
                    <th>Mã số phiếu xác nhận thực tập</th>
                    <th>Ngày hết hạn</th>
                    <th>Mã số sinh viên</th>
                    <th>Mã số giảng viên</th>
                    <th>Mã số cán bộ</th>
                    <th>Mã số đơn vị thực tập</th>
                    <th>Số đợt thực tập</th>
                </tr>
            </tfoot>
        </table>
    </body> 
</html> 

    


