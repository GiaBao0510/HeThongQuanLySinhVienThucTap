<?php
    ...
    $ketQua = intval(mysqli_fetch_array(TruyVan($lenhTim))['thay']);
    $KhongThay = "";
    ...
    if($ketQua > 0){
        ...
        if($chuCaiDau === "B" && $get_TaiKhoan['UserRole'] == 1){
            header('Location: ../SinhVien/TrangChuSinhVien.php');
        }elseif($chuCaiDau === "cbhd" && $get_TaiKhoan['UserRole'] == 4){
            header('Location: ../CanBoHuongDan/TrangChuCanBoHuongDan.php');
        }elseif($chuCaiDau === "gvhd" && $get_TaiKhoan['UserRole'] == 2){
            header('Location: ../GiaoVienHuongDan/TrangChuGiaoVien.php');
        }elseif($chuCaiDau === "dvtt" && $get_TaiKhoan['UserRole'] == 3){
            header('Location: ../DonViThucTap/TrangChuDVTT.php');
